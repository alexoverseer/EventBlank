...
class ViewModelBuilder: NSObject {
    ...
    func buildConferenceViewModels() -> [ConferenceViewVModel] {
        ...
        let viewModels = conferences.compactMap { buildConferenceViewModel(uid: $0.uid) }
        return viewModels
    }
    ...
    func buildConferenceViewModel(uid: String) -> ConferenceViewVModel? {
        ...
        let talks = talksRepository.getBy(conference: conference.uid).compactMap { buildTalkViewModel(uid: $0.uid) }
        let peopleIds = talks.map { $0.speaker.uid }
        let people = speakerRepository.getBy(group: peopleIds).compactMap { buildSpeakerViewModel(uid: $0.uid)}
